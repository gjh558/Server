...
#define FF_CMP_CHROMA 256
    ...
    /**
     * callback to negotiate the FFPixelFormat.
     ...
     */
    enum FFPixelFormat (*get_format)(struct AVCodecContext *s, const enum FFPixelFormat * fmt);
...
typedef struct AVCodec {
    ...
    const enum FFPixelFormat *pix_fmts;       ///array of supported pixel formats, or NULL if unknown, array is terminanted by -1
} AVCodec;
...
enum FFPixelFormat avcodec_get_pix_fmt(const char* name);
unsigned int avcodec_pix_fmt_to_codec_tag(enum FFPixelFormat p);
...
enum FFPixelFormat avcodec_default_get_format(struct AVCodecContext *s, const enum FFPixelFormat * fmt);
